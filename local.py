...
class SLAM:
    ...
    def localize_image(self, image_path):
        ...
        for kf_id in all_kf_ids:
            ...
            print(f"   KF#{kf_id}: conf={match_confidence*100:.1f}%, inliers={inlier_count}")

            # --- PRIORITY: confidence first, inliers only as tie-breaker ---
            if(match_confidence *100<10):
                continue    
            if (match_confidence > best_confidence) or (
                ...
                best_pnp_pose = {
                    'rvec': rvec.flatten() if success else None,
                    'tvec': tvec.flatten() if success else None,
                    'keyframe_pose': kf.pose.copy()
                }

        if best_match_kf_id is not None:
            print(f"\n✅ BEST MATCH: KF#{best_match_kf_id}  (conf={best_confidence*100:.1f}%, inliers={best_inliers})")
