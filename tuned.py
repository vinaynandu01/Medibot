# ===============================================================================
# TUNED SLAM: DISTANCE-BASED KEYFRAME SELECTION
# ===============================================================================
# Changes:
# 1. Distance-based cooldown: Keyframe every 0.2m instead of frame-based
# 2. Forward motion distance: 0.004m → 0.008m per frame
# ===============================================================================
...
class MapBuilder:
    def __init__(self):
        self.keyframes = {}  # Store all keyframes
        self.keyframe_directions = {}  # Store direction to next keyframe
    
    def add_keyframe(self, frame_id, position, orientation, features, descriptors):
        """Add a new keyframe to the map with direction to next keyframe"""
        keyframe = {
            'id': frame_id,
        ...
        # Calculate direction to next keyframe if there's a previous one
        if len(self.keyframes) > 0:
            prev_frame_id = list(self.keyframes.keys())[-1]
            direction_info = self._calculate_direction_to_next(
                self.keyframes[prev_frame_id], keyframe
            )
            self.keyframe_directions[prev_frame_id] = direction_info
        
        self.keyframes[frame_id] = keyframe
        return keyframe

    def _calculate_direction_to_next(self, prev_keyframe, next_keyframe):
        """Calculate direction and rotation angle from previous to next keyframe"""
        # Calculate relative position
        rel_pos = next_keyframe['position'] - prev_keyframe['position']
        
        # Calculate angle difference in orientation (yaw)
        prev_yaw = np.arctan2(prev_keyframe['orientation'][1], prev_keyframe['orientation'][0])
        next_yaw = np.arctan2(next_keyframe['orientation'][1], next_keyframe['orientation'][0])
        angle_diff = np.degrees(next_yaw - prev_yaw)
        
        # Normalize angle to [-180, 180]
        angle_diff = (angle_diff + 180) % 360 - 180
        
        # Determine direction type based on movement and rotation
        distance_moved = np.linalg.norm(rel_pos)
        
        # Classify the movement
        if distance_moved > 0.1:  # Significant forward movement
            if abs(angle_diff) < 22.5:  # Less than 22.5 degrees
                direction = 'forward'
                angle = 0
            elif angle_diff > 0:  # Left rotation while moving
                direction = 'forward_left'
                angle = self._snap_angle(abs(angle_diff))
            else:  # Right rotation while moving
                direction = 'forward_right'
                angle = self._snap_angle(abs(angle_diff))
        else:  # Primarily rotation in place
            if angle_diff > 0:
    ...
    def get_direction_to_next(self, frame_id):
        """Get the direction information to reach next keyframe from given frame"""
        return self.keyframe_directions.get(frame_id, None)
    
    def get_nearest_keyframe_direction(self, current_position, current_orientation):
        """Get direction to move from current position to nearest keyframe"""
        if len(self.keyframes) == 0:
        ...
        for frame_id, keyframe in self.keyframes.items():
            dist = np.linalg.norm(keyframe['position'] - current_position)
            if dist < min_dist:
                min_dist = dist
                nearest_frame_id = frame_id
        
        # If very close to nearest keyframe, get direction to next keyframe
        if min_dist < 0.5:  # Within 0.5m of keyframe
            next_direction = self.get_direction_to_next(nearest_frame_id)
            if next_direction:
                return {
                    'direction': next_direction['direction'],
                    'angle': next_direction['angle'],
                    'distance': next_direction['distance'],
                    'target_frame': nearest_frame_id,
                    'status': 'at_keyframe'
                }
            else:
        ...
        # Calculate direction to nearest keyframe
        nearest_keyframe = self.keyframes[nearest_frame_id]
        rel_pos = nearest_keyframe['position'] - current_position
    ...
    def print_direction_map(self):
        """Print the direction map for debugging"""
        print("\n=== Keyframe Direction Map ===")
        for frame_id in sorted(self.keyframes.keys()):
            direction_info = self.get_direction_to_next(frame_id)
            if direction_info:
                print(f"Frame {frame_id} -> Next: {direction_info['direction']} "
                      f"{direction_info['angle']}° (distance: {direction_info['distance']:.2f}m)")
        print("=" * 40)
# =============== Stream Capture ===============
...
# =============== DISTANCE-BASED KEYFRAME SLAM ===============
class TunedFeatureTrackingSLAM:
    """SLAM with distance-based keyframe cooldown"""
    ...
    def __init__(self, fx=500.0, fy=500.0, cx=320.0, cy=240.0,
                 ...
                 keyframe_distance_cooldown=0.2,  # NEW: Distance-based cooldown
                 local_map_size=10):
        ...
        self.FIXED_FORWARD_DISTANCE = 0.008  # CHANGED: 0.004 → 0.008
        ...
        self.keyframe_distance_cooldown = keyframe_distance_cooldown  # NEW: Distance-based
        self.distance_at_last_keyframe = 0.0  # Track total distance at last KF
        ...
        # Map builder for navigation directions
    ...
    # ======== DISTANCE-BASED KEYFRAME DECISION ========
    def _should_create_keyframe(self, coverage):
        """
        Create keyframe if ANY criteria met AND distance cooldown elapsed
        """
        
        # Check distance-based cooldown
        distance_since_kf = self.path_distance - self.distance_at_last_keyframe
        
        if distance_since_kf < self.keyframe_distance_cooldown:
            self.keyframe_cooldown_reason = f"Cooldown ({distance_since_kf:.3f}m/{self.keyframe_distance_cooldown:.2f}m)"
            return False
    ...
    def _create_keyframe(self, gray, depth_map, kp_orb, desc_orb, features_3d, keypoints_2d):
        ...
        self.distance_at_last_keyframe = self.path_distance  # Track distance at KF
        self.keyframe_counter += 1

        self.keyframe_trajectory.append(self.pose.copy())
        
        # Add keyframe to MapBuilder for navigation directions
        position = np.array([self.pose[0], self.pose[1]])
        orientation = np.array([np.cos(self.pose[2]), np.sin(self.pose[2])])
    ...
    def draw_top_view(self, scale=100, size=900, fps=None):
        ...
        y += 30
        distance_since_kf = self.path_distance - self.distance_at_last_keyframe
        cv2.putText(canvas, f"Cooldown: {distance_since_kf:.3f}m/{self.keyframe_distance_cooldown:.2f}m", 
    ...
    def run_slam(self, source=0, target_w=640, target_h=480,
        ...
        print("\n" + "="*110)
        print("🎯 DISTANCE-BASED KEYFRAME SLAM")
        print("="*110)
        print(f"✅ Keyframe triggers (ANY met + distance cooldown):")
        print(f"   1. Tracked features < {self.tracked_feature_threshold}")
        print(f"   2. Distance traveled > {self.distance_threshold:.3f}m")
        print(f"   3. Rotation > {math.degrees(self.rotation_threshold):.1f}°")
        print(f"   4. Feature coverage < {self.feature_coverage_threshold*100:.0f}%")
        print(f"   + Minimum {self.keyframe_distance_cooldown:.2f}m between keyframes")
        print(f"✅ Forward step: {self.FIXED_FORWARD_DISTANCE:.4f}m per frame")
        ...
        while True:
            ...
            cv2.imshow("Camera View - Distance-Based Keyframes", vis)
            
            top_map = self.draw_top_view(scale=100, size=900, fps=fps)
            cv2.imshow("Top View - Distance-Based Keyframe Selection", top_map)
            ...
            if key == 27:
                ...
                map_filename = f"slam_distance_{timestamp}.png"
                ...
                print(f"   Cooldown setting: {self.keyframe_distance_cooldown:.2f}m\n")
            elif key == ord('+') or key == ord('='):
                self.keyframe_distance_cooldown += 0.05
                print(f"⬆️ Cooldown increased to {self.keyframe_distance_cooldown:.2f}m (fewer keyframes)")
            elif key == ord('-') or key == ord('_'):
                self.keyframe_distance_cooldown = max(0.05, self.keyframe_distance_cooldown - 0.05)
                print(f"⬇️ Cooldown decreased to {self.keyframe_distance_cooldown:.2f}m (more keyframes)")
        ...
        # Save navigation direction map
        self._save_direction_map()
        
    def _save_direction_map(self):
        """Save the navigation direction map to a JSON file"""
        try:
            direction_map_file = os.path.join(self.storage_manager.storage_dir, "direction_map.json")
            direction_data = {
                'keyframe_directions': self.map_builder.keyframe_directions,
                'total_keyframes': len(self.map_builder.keyframes),
                'timestamp': time.strftime("%Y-%m-%d %H:%M:%S")
            }
            
            with open(direction_map_file, 'w') as f:
                json.dump(direction_data, f, indent=2)
            
            print(f"✅ Direction map saved: {direction_map_file}")
            
            # Print direction map for debugging
            self.map_builder.print_direction_map()
            
        except Exception as e:
            print(f"⚠️ Could not save direction map: {e}")

# =============== MAIN ===============
def main_menu():
    print("\n" + "="*110)
    print("🗺️ DISTANCE-BASED KEYFRAME SLAM SYSTEM")
    print("="*110)
    print("1. Build Map (Distance-Based Keyframe Selection)")
    print("2. Exit")
    print("="*110)
...
def build_map_menu():
    ...
    cooldown = input("Keyframe distance cooldown [0.2m]: ").strip()
    keyframe_distance_cooldown = float(cooldown) if cooldown else 0.2
    ...
    slam = TunedFeatureTrackingSLAM(
        ...
        rotation_threshold=30.0,
        feature_coverage_threshold=0.6,
        keyframe_distance_cooldown=keyframe_distance_cooldown,
        local_map_size=10
    )
