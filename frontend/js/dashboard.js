...
class Dashboard {
    ...
    async connectRover() {
        ...
        try {
            ...
            if (response.success) {
                ...
                this.showConnectionMessage('Connected successfully', 'success');
                this.startCameraFeed();
                this.logActivity(`Connected to rover at ${this.roverIP}`);
            }
        } catch (error) {
            console.error('Connection error:', error);
            this.showConnectionMessage('Connection failed: ' + error.message, 'error');
        } finally {
    ...
    updateConnectionStatus(connected) {
        ...
        const controlButtons = ['forwardBtn', 'backwardBtn', 'leftBtn', 'rightBtn', 'stopBtn'];
    ...
    updateRoverState(state) {
        ...
        if (state.current_keyframe !== null && state.current_keyframe !== undefined) {
            document.getElementById('currentKF').textContent = `KF#${state.current_keyframe}`;
        }
